...
START_TEST(test_create) {
  matrix_t create;
  int status = create_matrix(create_matrix_proportions[_i][0],
                             create_matrix_proportions[_i][1], &create);
  ck_assert_int_eq(create_return[_i], status);
  if (!status) remove_matrix(&create);
}

START_TEST(test_eq) {
  matrix_t value_1, value_2;
  int status_1 = create_matrix(3, 3, &value_1);
  int status_2 = create_matrix(3, 3, &value_2);
  static_to_dynamic(val_1[_i], &value_1);
  static_to_dynamic(val_2[_i], &value_2);
  ck_assert_int_eq(compare_results[_i], is_eq_matrix(&value_1, &value_2));
  if (!status_1) remove_matrix(&value_1);
  if (!status_2) remove_matrix(&value_2);
}
END_TEST
...
START_TEST(test_sum) {
  ...
  static_to_dynamic(val_1[_i], &value_1);
  static_to_dynamic(val_2[_i], &value_2);
  int status_result = sum_matrix(&value_1, &value_2, &result);
  compare_matrix(result, sum_result[_i]);
  ck_assert_int_eq(0, status_result);
  if (!status_1) remove_matrix(&value_1);
  if (!status_2) remove_matrix(&value_2);
  if (!status_result) remove_matrix(&result);
}
END_TEST
...
START_TEST(test_sub) {
  ...
  static_to_dynamic(val_1[_i], &value_1);
  static_to_dynamic(val_2[_i], &value_2);
  int status_result = sub_matrix(&value_1, &value_2, &result);
...
START_TEST(test_mult_number) {
  ...
  int status_1 = create_matrix(3, 3, &value_1);
  static_to_dynamic(val_1[_i], &value_1);
  int status_result = mult_number(&value_1, number_to_mult_by[_i], &result);
...
START_TEST(test_mult_matrix) {
  ...
  static_to_dynamic(val_1[_i], &value_1);
  static_to_dynamic(val_2[_i], &value_2);
  int status_result = mult_matrix(&value_1, &value_2, &result);
...
START_TEST(test_transpose) {
  ...
  int status_1 = create_matrix(3, 3, &value_1);
  static_to_dynamic(val_1[_i], &value_1);
  int status_result = get_transpose(&value_1, &result);
...
START_TEST(test_calc_complements) {
  ...
  int status_1 = create_matrix(3, 3, &value_1);
  static_to_dynamic(val_1[_i], &value_1);
  int status_result = get_calc_complements(&value_1, &result);
...
START_TEST(test_determinant) {
  ...
  static_to_dynamic(val_1[_i], &value_1);
  double det = 0;
  int status_result = get_determinant(&value_1, &det);
...
START_TEST(test_inverse) {
  ...
  int status_1 = create_matrix(3, 3, &value_1);
  static_to_dynamic(val_1[_i], &value_1);
  int status_result = inverse_matrix(&value_1, &result);
  if (!status_result) {
